...
source-hygiene: verify-source-package-policy
	...
	@if ! rg -n 'func resultFromRollout\(intent deploymentv1\.RolloutIntent, record deploymentv1\.RolloutRecord' agent/internal/cloudrunner/result.go >/dev/null || ! rg -n 'record = preMutationFailureRecord\(intent, reconcileErr\)' agent/internal/cloudrunner/result.go >/dev/null; then echo "pre-WAL rollout result is not anchored to the leased intent"; exit 1; fi
	@if rg -n 'terminal failed rollout requires NO_KNOWN_GOOD' cloud/internal/registry/rollout.go; then echo "pre-mutation rollout failure is incorrectly limited to NO_KNOWN_GOOD"; exit 1; fi
	@if ! rg -n 'result\.FailureCode == ""' cloud/internal/registry/rollout.go >/dev/null || ! rg -n 'result\.FailureCode == deploymentv1\.RolloutCodeNoKnownGood' cloud/internal/registry/rollout.go >/dev/null; then echo "typed pre-mutation and NO_KNOWN_GOOD validation split is missing"; exit 1; fi
