...
## Implemented Runtime
  ...
  - Incident path supports sanitized deterministic context from bounded local telemetry metric/log fingerprint windows plus list/get/resolve, project/role authorization, MTTR, and resolve audit. Agent incident runtime has no HTTP AI analyzer, fallback RCA, provider/model/fallback metadata, AI network call, RCA-backed approval path, or Kubernetes mutation executor. The legacy `AnalyzeIncident` and `ApproveIncidentAction` RPCs return `Unimplemented` until V3-004. Historical RCA/mitigation columns remain storage-only for upgrade compatibility and are not exposed or executed.
  ...
  - Local incident list/detail/analyze/approve/resolve endpoints remain wired Browser -> CLI local backend -> Agent gRPC IncidentService pending V3-004 cleanup. List/detail/resolve remain functional; analyze and approve now fail closed with `Unimplemented` at the Agent and cannot authorize or execute mutations.
...
- Cloud AI runtime and Agent-side AI analyzer/fallback/metadata/RCA-backed execution paths are removed. Contracts/CLI/UI analyze and approve surfaces remain for V3-004, but both transitional Agent RPCs fail closed with `Unimplemented`. The future CLI-side user-owned AI bridge remains unimplemented.
