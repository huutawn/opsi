...
## Implemented Runtime
  ...
  - Incident path supports sanitized deterministic context from bounded local telemetry metric/log fingerprint windows plus list/get/resolve, project/role authorization, factual incident fields, MTTR, and resolve audit. The active contract and service descriptor expose only list/get/resolve. Agent incident runtime has no HTTP AI analyzer, fallback RCA, provider/model/fallback metadata, AI network call, RCA-backed approval path, analyze/approve RPC, or Kubernetes mutation executor. Historical RCA/mitigation columns remain storage-only for upgrade compatibility and are not exposed or executed.
  ...
  - Local incident list/detail/resolve endpoints are wired Browser -> CLI local backend -> Agent gRPC IncidentService. Resolve remains protected by local session and idempotency requirements. Analyze/approve CLI commands, Local API routes, client methods, UI controls and RCA/provider/recommended-action fields are removed; removed local paths return 404.
...
- Local/dev runtime for OTP and PAT verify. Cloud AI runtime, providers, Gemini integration, fixture RCA, and `/v1/ai/*` routes have been removed.
  ...
  - Cloud does not own AI runtime or AI providers. Roadmap v3 assigns future user-owned AI integration to a CLI-side bridge, which is not implemented yet. The legacy incident AI and remediation coupling has been removed.
  - Root README documents clean-checkout verification, Go `GOTOOLCHAIN=local`, module test commands, optional `` wrapper fallback, and offline cache expectations. Root Makefile exposes `verify`, `test`, `verify-postgres`, `build`, `clean`, and `package-source`; `verify` covers Go vet/tests plus UI `npm ci`, build/typecheck, and lint; `verify-postgres` requires `OPSI_TEST_DATABASE_URL` and runs the Postgres-backed registry/relay restart, retry, dead-letter, idempotency, audit, and redaction tests with skip prevention. Source packaging uses the Git-aware tracked plus untracked/non-ignored file set, rejects local configs/private-key material/runtime state/generated output, and writes `dist/opsi-source.tar.gz` atomically. Release output is recreated before approved binaries, docs, checksums, and sanitized example configs are copied and checked.
  - Runtime security hardening includes Agent production/non-loopback config fail-fast, Cloud production OTP/dev-echo and HTTPS public URL guards, Agent request HMAC compatibility, omitted OTP `code` outside dev echo, secret reveal second-factor negative tests, and a proto-vs-handwritten-Go service/RPC drift test.
  ...
  - Clean VPS/K3s E2E now has a canonical protected command path: `make verify-e2e-k3s-preflight`, `make verify-e2e-k3s`, `scripts/e2e/verify-k3s.sh`, sample good/bad services under `test/e2e/`, a manual GitHub workflow, and a runbook. It validates factual incident list/get/resolve only. No full real-infra pass artifact is recorded yet.
...
- Cloud AI runtime and Agent-side AI analyzer/fallback/metadata/RCA-backed execution paths are removed. The active incident contract and user-facing surfaces now contain only list/get/resolve. The future CLI-side user-owned AI bridge and safe ActionPlane remain unimplemented.
- The legacy ingress remediation action and Nginx-specific request-limit annotation are not part of the active runtime. Opsi currently has no incident-derived remediation action plane; generic Cloud/API rate limits remain active security controls. Future gateway policy adapters are not implemented.
- Opsi does not currently generate or manage Kubernetes Ingress, Gateway API routes, domains, or TLS certificates. The misleading deployment ingress option and its automatic preStop sleep were removed; user-supplied manifests may still define their own Service, Ingress, Gateway, lifecycle, and shutdown behavior. Managed gateway exposure remains planned for Phase 4.
...
- Tracked local Agent/Cloud configuration has been removed from the current tree. Source and release hygiene gates reject local configuration, environment/credential paths, private-key material, runtime certificates, databases, logs, and generated output. Previously committed local configuration remains in Git history; V3-007 does not rewrite history or rotate credentials.
