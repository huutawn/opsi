...
- Local/dev runtime for OTP, PAT verify, fixture AI RCA, and Gemini RCA adapter with explicit fallback metadata.
  ...
  - `opsi-bootstrap-worker` exists as a standalone fail-closed worker: it validates production config, takes one bootstrap credential envelope, validates target metadata, records/audits truthful bootstrap states, redacts output, then returns `BOOTSTRAP_RUNTIME_UNSUPPORTED` and marks the session failed because SSH/K3s/Agent installation is not implemented. Cloud rejects SSH private-key upload, and Agent heartbeat moves bootstrap to `verifying` rather than terminal completion.
