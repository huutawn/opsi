...
export function ApplicationWizard({ console, onClose, onCreated }: { console: ConsoleController; onClose: () => void; onCreated?: () => void | Promise<void> }) {
  ...
  function reviewApplication(event: FormEvent<HTMLFormElement>) {
	...
	const applicationRoot = String(form.get("application_root") || ".").trim();
    ...
    console.reviewMutation(
      ...
      async (key) => {
        ...
        await client.createGitHubBinding(projectID, { service_id: created.id, repository_id: repository.repository_id, service_key: serviceKey, config_path: configPath, selected_ref: branch, application_root: applicationRoot, build_context: buildContext, build_strategy: "dockerfile", dockerfile_path: dockerfile }, key);
        await Promise.all([
