...
export function ApplicationWizard({ console, onClose, onCreated, resumeService }: { console: ConsoleController; onClose: () => void; onCreated?: () => void | Promise<void>; resumeService?: ServiceRecord }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([client.githubInstallations(projectID), client.githubRepositories(projectID), client.githubBindings(projectID), client.services(projectID)])
      .then(([installationData, repositoryData, bindingData, serviceData]) => {
        ...
        const resumeRepository = resumeService?.repo_url ? nextRepositories.find((item) => `https://github.com/${item.full_name}` === resumeService.repo_url) : undefined;
        const firstRepository = resumeRepository ?? nextRepositories.find((item) => item.installation_id === firstInstallation?.installation_id && repositoryUsable(item));
        setInstallations(nextInstallations);
        setRepositories(nextRepositories);
        setBindings((bindingData.bindings ?? []).filter((item) => item.status === "active"));
        setServices(serviceData.services ?? []);
        setInstallationID(firstRepository?.installation_id ?? firstInstallation?.installation_id ?? 0);
        setRepositoryID(firstRepository?.repository_id ?? 0);
        setSelectedRef(resumeService?.branch || firstRepository?.default_branch || "main");
        setServiceKey(resumeService?.name ?? "");
        if (resumeService && firstRepository) setStep("application");
        setLoading(false);
      })
      .catch((cause: unknown) => {
        if (!active) return;
  ...
  function reviewApplication(event: FormEvent<HTMLFormElement>) {
    ...
    const strategyLabel = buildStrategy === "buildpack" ? "Cloud Native Buildpacks" : buildStrategy === "dockerfile" ? `Dockerfile ${dockerfilePath}` : "Automatic";
    const mutations = [
