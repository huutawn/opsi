  ...
  return (
    <main className="mx-auto max-w-7xl space-y-6 p-4 lg:p-margin-desktop">
      ...
      {sourceOnly ? <SourceStep busy={busy} canMutate={canMutate && !isAuthFailure(error)} hostname={hostname} installationID={installationID} installations={installations} linkedInstallationIDs={linkedInstallationIDs} onConnectInstallation={() => void connectInstallation()} onDiscover={() => void discoverGitHub()} onHostname={(value) => { setHostname(value); updateSourceDraft(projectID, { hostname: value }); }} onInstallation={(value) => { setInstallationID(value); updateSourceDraft(projectID, { installationID: value }); }} onRef={(value) => { setRefName(value); updateSourceDraft(projectID, { refName: value }); }} onRepository={(value) => { setRepositoryID(value); updateSourceDraft(projectID, { repositoryID: value }); }} onStart={() => void sourceAction()} quotaBlocked={hostnameQuotaBlocked(hostname)} refName={refName} repositories={repositories} repositoryID={repositoryID} /> : run && <>
        ...
        <PrimaryAction bootstrapActive={bootstrapActive} busy={busy} canMutate={canMutate} connectTrigger={connectTrigger} draftDirty={draftDirty} needsServer={needsServer} onAction={action} onConnect={() => setShowConnect(true)} onNew={() => { setShowNew(true); setHostname(""); setError(null); }} onSaveDraft={() => void saveDraft()} run={run} />
...
function PrimaryAction({ bootstrapActive, busy, canMutate, connectTrigger, draftDirty, needsServer, onAction, onConnect, onNew, onSaveDraft, run }: { bootstrapActive: boolean; busy: string; canMutate: boolean; connectTrigger: RefObject<HTMLButtonElement | null>; draftDirty: boolean; needsServer: boolean; onAction: (name: "analyze" | "approve" | "acknowledge" | "retry" | "cancel", body?: Record<string, unknown>) => void; onConnect: () => void; onNew: () => void; onSaveDraft: () => void; run: DeploymentRun }) {
  let label = "";
  ...
  return <div className="sticky bottom-4 z-10 flex items-center justify-between gap-4 border border-outline-variant/40 bg-surface-container-high p-4 shadow-lg"><p className="text-sm text-on-surface-variant">{bootstrapActive ? "Opsi is connecting and verifying the server." : newDeployment ? "This run reached its bounded retry limit." : saveDraftFirst ? "Save the edited draft before approving the immutable deployment plan." : "This is the only action needed for the current state."}</p><Button disabled={Boolean(busy) || bootstrapActive} onClick={() => saveDraftFirst ? onSaveDraft() : action ? onAction(action, body) : newDeployment ? onNew() : onConnect()} ref={!action && !newDeployment && !saveDraftFirst ? connectTrigger : undefined} size="lg" variant={action === "cancel" ? "outline" : "primary"}>{working ? "Saving…" : label}</Button></div>;
}
