...
const starters = [
  "Review this project for Opsi deployment readiness and list the highest-risk gaps.",
  "Review how the frontend should call every backend through one public hostname.",
  "Review application configuration variables and propose safe non-secret values.",
];
...
export function AssistantView({ console }: { console: ConsoleController }) {
  ...
  useEffect(() => {
    ...
    }).catch((cause) => active && setFailure(errorMessage(cause)));
    return () => { active = false; };
  }, [client]);
        ...
        if (next.state === "succeeded") {
          setMessages((current) => [
            ...current,
            {
              id: next.id,
              role: "assistant",
              text: next.response || "No response.",
  ...
  async function applySourcePatch(turnID: string, proposal: AssistantSourcePatchProposal) {
    ...
    if (!window.confirm(`Apply this verified patch to your local worktree at ${proposal.source_commit}?\n\nFiles: ${files}\n\nOpsi will not test, commit, push, or create a PR.`)) return;
    ...
    try {
      ...
      setMessages((current) => current.map((message) => message.id === turnID ? { ...message, text: `${message.text}\n\nLocal source patch applied. Review the diff, then test and commit it yourself.` } : message));
    } catch (cause) {
      setFailure(errorMessage(cause));
    } finally {
      setPatchBusy("");
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-6 p-4 lg:p-margin-desktop">
      <PageHeader
        ...
        icon="hub"
        title="AI Assistant"
        description="Review your project with an agent grounded in Opsi's current, read-only MCP facts."
      />
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="font-semibold" id="agent-connection-title">Agent connection</h2>
                <StatusBadge
                  label={overallReady ? "Connected" : "Needs setup"}
              ...
              <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-on-surface-variant">
                <span className="flex items-center gap-1.5">
                  ...
                  <span>{providerReady ? "Provider authenticated" : "Provider needs setup"}</span>
                </span>
                <span>·</span>
                <span className="flex items-center gap-1.5">
                  <span className={`inline-block h-2 w-2 rounded-full ${cloudPATValid ? "bg-status-ready" : "bg-status-failed"}`} />
                  <span>{cloudPATValid ? "Opsi Cloud PAT valid" : "Cloud unauthenticated"}</span>
                </span>
                <span>·</span>
                <span className="flex items-center gap-1.5">
                  <span className={`inline-block h-2 w-2 rounded-full ${groundingVerified ? "bg-status-ready" : "bg-on-surface-variant/40"}`} />
                  <span>{groundingVerified ? "MCP grounding verified" : "MCP grounding ready"}</span>
                </span>
              </div>
        ...
        <section aria-labelledby="assistant-chat-title" className="flex min-h-[560px] flex-col border border-outline-variant/30 bg-surface-container-low">
          ...
          <div aria-live="polite" className="flex-1 space-y-5 overflow-y-auto p-4 sm:p-6">
            {messages.length === 0 && (
              <div className="mx-auto max-w-xl py-12 text-center">
                ...
                <h3 className="mt-4 text-lg font-semibold">Ask from current Opsi facts</h3>
          ...
          <form className="border-t border-outline-variant/30 bg-surface-container p-4" onSubmit={submit}>
            ...
            <Textarea
              ...
              placeholder={overallReady ? "Ask for a project review or configuration recommendation…" : "Connect and authenticate a local AI agent and Opsi Cloud session first."}
              rows={3}
              value={prompt}
            />
  ...
  return (
    <section aria-label={`Configuration proposal for ${proposal.application_name}`} className="mt-4 border border-secondary/40 bg-secondary/5 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium uppercase tracking-wider text-secondary">Configuration proposal</p>
          <h3 className="mt-1 font-semibold">{proposal.application_name}</h3>
        </div>
        <StatusBadge
          className="!text-on-surface"
          label={review ? review.review.status.replaceAll("_", " ") : "Agent draft (validated)"}
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        ...
        {!review && !canMutate && (
          <p className="text-xs text-on-surface-variant">View-only role: ask a developer or owner to create review.</p>
        )}
...
function SourcePatchCard({ canMutate, onApply, proposal, working }: { canMutate: boolean; onApply: () => void; proposal: AssistantSourcePatchProposal; working: boolean }) {
  ...
  return (
    <section aria-label="Validated local source patch" className="mt-4 border border-status-warning/40 bg-status-warning/5 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-medium uppercase tracking-wider text-status-warning">Validated source patch</p>
      ...
      <div className="mt-4">
        {canMutate ? <Button disabled={working} onClick={onApply} variant="secondary"><Icon name="edit_note" />Apply to local worktree</Button> : <p className="text-xs text-on-surface-variant">View-only role: ask a developer or owner to apply this local patch.</p>}
      </div>
    </section>
  );
}
...
function errorMessage(cause: unknown) {
  return cause instanceof LocalAPIError || cause instanceof Error ? cause.message : "Unexpected local assistant error.";
}
