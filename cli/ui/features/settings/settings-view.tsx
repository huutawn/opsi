...
export function SettingsView({ console }: { console: ConsoleController }) {
  ...
  return <section className="settingsPage">
    ...
    <Tabs label="Settings sections" items={groupedTabs.settings.map((item) => ({ ...item, href: routeHref({ ...console.route, tab: item.id }) }))} selected={tab} onSelect={(event, next) => { if (event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return; event.preventDefault(); console.navigate({ tab: next }); }} />
    <div {...tabPanelProps("Settings sections", tab)}>
    {tab === "general" ? <section className="settingsSection" aria-labelledby="settings-session"><div className="sectionHeading"><div><h2 id="settings-session">General</h2><p>Current Local workspace and selected project context.</p></div><StatusBadge value={console.session?.authenticated ? "healthy" : "unavailable"} /></div><dl className="settingsFacts"><Fact label="State" value={console.session?.authenticated ? "Authenticated" : "Signed out"} /><Fact label="Organization" value={console.session?.org_id || "Not reported"} /><Fact label="Selected project context" value={console.state.project ? `${console.state.project.name} (${console.state.project.id})` : "None"} /><Fact label="Token status" value={console.session?.token_status || "Unknown"} /></dl></section> : null}
    {tab === "authentication" ? <section className="settingsSection" aria-labelledby="settings-token"><div className="sectionHeading"><div><h2 id="settings-token">Authentication</h2><p>PAT lifecycle uses the existing Local session boundary; token material is never shown.</p></div></div><div className="tokenActions"><button className="primary" disabled={!console.session?.authenticated} onClick={rotate} type="button">Review PAT rotation</button><button disabled={!console.session?.authenticated} onClick={revoke} type="button">Review revoke and sign out</button><button onClick={() => void console.actions.load()} type="button">Refresh status</button></div></section> : null}
    {tab === "integrations" ? <section className="settingsSection" aria-labelledby="settings-connections"><div className="sectionHeading"><div><h2 id="settings-connections">Integrations</h2><p>Connection and trust facts without credential material.</p></div></div><dl className="settingsFacts"><Fact label="Cloud connection" value={connectionLabel(console.session?.cloud_connected)} status={connectionStatus(console.session?.cloud_connected)} /><Fact label="Agent connection" value={connectionLabel(console.session?.agent_connected)} status={connectionStatus(console.session?.agent_connected)} /><Fact label="Cloud authority" value={settings.cloud_authority || "Missing"} /><Fact label="Agent TLS pin" value={settings.agent_tls_pinned ? "Configured" : "Missing"} status={settings.agent_tls_pinned ? "healthy" : "unknown"} /></dl></section> : null}
    {tab === "system" ? <><section className="settingsSection" aria-labelledby="settings-version"><div className="sectionHeading"><div><h2 id="settings-version">System</h2><p>Build and installed-asset facts reported by the Local backend.</p></div></div><dl className="settingsFacts"><Fact label="CLI/UI version" value={settings.version || "Missing"} /><Fact label="Revision" value={settings.revision || "Missing"} mono /><Fact label="Go version" value={settings.go_version || "Missing"} /><Fact label="Installed UI assets" value={settings.ui_assets || "Missing"} /><Fact label="Configuration selected" value={settings.config_selected ? "Configured" : "Missing"} status={settings.config_selected ? "healthy" : "unknown"} /></dl></section><details className="capabilityLimits"><summary>Capability limits</summary>{settings.backend_gaps.length ? <dl>{settings.backend_gaps.map((gap) => <div key={gap.capability}><dt>{gap.capability}</dt><dd>{gap.status}. Roadmap: {gap.roadmap}.</dd></div>)}</dl> : <Empty text="No backend capability limits were reported." />}</details></> : null}
    </div>
  </section>;
}
