...
function BootstrapTab({ console, onReload }: { console: ConsoleController; onReload: () => Promise<void> }) {
  ...
  return <div className="bootstrapLayout"><section className="bootstrapInventory"><div className="sectionHeading"><div><h2>Bootstrap sessions</h2><p>Durable Cloud worker sessions; no public DNS assumption.</p></div></div>{console.state.sessions.length ? <ul>{console.state.sessions.map((session) => <li key={session.id}><button aria-pressed={selected?.id === session.id} onClick={() => { console.navigate({ session: session.id }); void console.actions.loadBootstrapEvents(session.id); }} type="button"><span><strong>{session.public_host || session.id}</strong><small>{session.role} · attempt {session.attempt_count ?? "?"}/{session.max_attempts ?? "?"}</small></span><StatusBadge value={session.status} /></button></li>)}</ul> : <Empty title="No bootstrap sessions" text="Use Connect Server to create a reviewed bootstrap session." />}</section><section className="bootstrapDetail">{selected ? <><div className="detailHeading"><div><p className="eyebrow">Selected session</p><h2>{selected.id}</h2></div><StatusBadge value={selected.status} /></div><dl className="evidenceGrid"><Fact label="Attempt" value={`${selected.attempt_count ?? "Unknown"}/${selected.max_attempts ?? "Unknown"}`} /><Fact label="Progress" value={progress.percent === null ? progress.label : `${progress.percent}% · ${progress.label}`} /><Fact label="Next step" value={selected.checkpoint ? `Step ${selected.checkpoint.next_step_index}` : "Not reported"} /><Fact label="Failure" value={selected.last_failure_message_redacted || selected.last_failure_code || "None reported"} /></dl>{selected.id === console.state.bootstrapCommandSessionID && console.state.bootstrapCommand ? <BootstrapCommand command={console.state.bootstrapCommand} /> : selected.status === "waiting" && selected.auth_method === "command" ? <p className="notice" role="status">Waiting for connection. This browser only shows the command when it is issued.</p> : null}<EventTimeline events={console.state.bootstrapEventsSessionID === selected.id ? console.state.bootstrapEvents : []} /><button disabled={!['failed', 'dead_letter'].includes(selected.status)} onClick={() => console.actions.retryBootstrap(selected.id, onReload)} type="button">Retry eligible session</button></> : <Empty text="Select a bootstrap session." />}</section></div>;
}
...
function BootstrapDialog({ console, onClose, onCreated }: { console: ConsoleController; onClose: () => void; onCreated: () => Promise<void> }) {
  ...
  return <dialog aria-describedby="bootstrap-description" aria-labelledby="bootstrap-title" className="placementDialog" onCancel={(event) => { event.preventDefault(); onClose(); }} ref={dialog}><div className="dialogHeading"><div><p className="eyebrow">Topology · Connect Server</p><h2 id="bootstrap-title">Connect Server</h2><p id="bootstrap-description">Create a reviewed one-time bootstrap command, run it as root on the VPS, and keep this page open for factual progress.</p></div><button aria-label="Close connect server dialog" autoFocus className="iconButton" onClick={onClose} type="button"><svg aria-hidden="true" viewBox="0 0 20 20"><path d="m5 5 10 10M15 5 5 15" /></svg></button></div><form className="form placementForm" onSubmit={(event) => { void console.actions.addServer(event, onCreated); onClose(); }}><label>Role<select className="select" defaultValue="first_server" name="role" required><option value="first_server">First server</option><option value="worker">Worker</option></select></label><label>Server IP or hostname<input autoComplete="off" className="field" name="public_host" placeholder="203.0.113.10" required spellCheck={false} /></label><fieldset className="bootstrapMethod span2"><legend>Connection method</legend><label className="methodChoice"><input checked={method === "command"} name="auth_method" onChange={() => setMethod("command")} type="radio" value="command" /><span><strong>Run bootstrap command</strong><small>Recommended. Opsi never receives an SSH private key or password.</small></span></label><details><summary>Advanced: Bootstrap over SSH</summary><div className="advancedBootstrap"><label className="methodChoice"><input checked={method === "password"} name="auth_method" onChange={() => setMethod("password")} type="radio" value="password" /><span><strong>SSH password</strong><small>Requested again only after review.</small></span></label><label className="methodChoice"><input checked={method === "private_key"} name="auth_method" onChange={() => setMethod("private_key")} type="radio" value="private_key" /><span><strong>SSH private key</strong><small>Uses the existing verified known_hosts workflow.</small></span></label>{method !== "command" ? <div className="sshBootstrapFields"><label>SSH port<input className="field" defaultValue="22" inputMode="numeric" max="65535" min="1" name="ssh_port" required type="number" /></label><label>SSH username<input autoComplete="username" className="field" defaultValue="root" name="ssh_username" required spellCheck={false} /></label></div> : null}</div></details></fieldset><p className="notice span2">The command contains an expiring token scoped to this reviewed project, environment, runtime, and session. It can claim this session once.</p><div className="dialogActions span2"><button onClick={onClose} type="button">Cancel</button><button type="submit">Review connection</button></div></form></dialog>;
}
...
function BootstrapCommand({ command }: { command: string }) {
  ...
  return <section aria-labelledby="bootstrap-command-title" className="bootstrapCommand"><div><div><p className="eyebrow">One-time command</p><h4 id="bootstrap-command-title">Run on the VPS as root</h4></div><button onClick={() => void copy()} type="button">{copyState}</button></div><code>{command}</code><p role="status">Waiting for connection. Progress updates here after the command claims the session.</p></section>;
}
