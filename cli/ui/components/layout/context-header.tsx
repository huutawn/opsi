...
export function ContextHeader({ environment, lastUpdated, menuButtonRef, onMenu, onRefresh, project, route, serviceScope, session }: {
  ...
  return <header className="contextHeader">
    ...
    <div className="contextIdentity" aria-label="Current context">
      <div className="breadcrumb" aria-label="Breadcrumb"><span>{project ? "Projects" : session.org_id || "Workspace unavailable"}</span>{project ? <><i aria-hidden="true">/</i><span title={project.name}>{project.name}</span><i aria-hidden="true">/</i><span title={environment}>{environment}</span><i aria-hidden="true">/</i><strong aria-current="page">{routeLabel(route)}</strong></> : null}</div>
