...
| Action approval devices | Cloud ActionDevice APIs | `opsi action device register|list|revoke` | mutation/read | owner/developer; viewer list only | sanitized JSON | 30s | required registration identity; idempotent revoke | Cloud registry/PostgreSQL/keychain tests | UI deferred to R5-017 | SOURCE_PASS |
| Safe runtime actions | Agent ActionService RPCs | `opsi action catalog|preflight|approve|execute|status` | read/mutation | owner/developer | sanitized JSON + exact TTY phrase | challenge max 5m; plan max 10m | nonce consume, exact terminal replay, target lock | contracts/service/replay/race/fake Kubernetes tests | UI deferred to R5-017 | SOURCE_PASS |
