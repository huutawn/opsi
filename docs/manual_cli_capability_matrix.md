...
| Node offline/drain/remove | `/nodes/{id}/{offline,drain,remove}` | `opsi node offline|drain|remove` | mutation | owner/admin | JSON/JSON | 30s | required key; offline replay is durable and target-bound | registry memory/PostgreSQL/HTTP and node/client tests | node lifecycle actions | SUPPORTED / SOURCE_IDEMPOTENCY_FIXED / LIVE_RETEST_PENDING |
