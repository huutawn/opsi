...
- **Build runner auth:** Cloud dispatches only configured Opsi executor workflows through the GitHub App, then accepts `opsi-build` GitHub OIDC from the exact configured repository, workflow, and ref. One atomic claim changes a Dockerfile BuildJob from `ready` to `running` and returns a ten-minute, BuildJob/attempt/run-scoped lease whose SHA-256 hash is stored separately from the immutable source snapshot.
