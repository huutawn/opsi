...
func TestNATSCompilerLeaseReadinessDeleteAndBinding(t *testing.T) {
	...
	evidence := &resourcev1.ManagedResourceEvidence{ObservedSpecHash: lease.Spec.SpecHash, WorkloadReady: true, PodReady: true, ServiceReady: true, Image: lease.Spec.Image, AvailableReplicas: 1, ObservedAt: time.Now().UTC()}
