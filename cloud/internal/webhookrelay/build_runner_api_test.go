...
func TestBuildRunnerClaimAndBuildSpecAPI(t *testing.T) {
	...
	result := buildjob.RunnerResult{BuildJobID: job.ID, AttemptID: attempt.AttemptID, RegistryReference: registryConfig.Target(job.ApplicationID, job.ID).DigestReference(digest), Digest: digest, Executor: buildjob.ExecutorResult{Strategy: buildjob.StrategyDockerfile, Platform: "linux/amd64", BuildKitVersion: "v0.32.2", BuildxVersion: "v0.36.1", BuilderIdentity: "moby/buildkit:v0.32.2@sha256:28a898719c18a33f4e8000685287fa36fd0dd9560c6440227d3a732d79bb41d8", StartedAt: time.Unix(201, 0).UTC(), CompletedAt: time.Unix(202, 0).UTC(), BuildDescriptor: descriptor, Remote: buildjob.RemoteRegistryEvidence{Descriptor: descriptor, Platform: "linux/amd64", Manifest: manifest, Private: true}}}
