...
<main class="main">
<div class="debug">Dev/debug Cloud console. Production UX is the Local Web UI served by <code>opsi start</code>.</div>
<div class="top">
...
function serviceForm(){
 return "<form id=\"serviceForm\" class=\"forms\"><label>Type<select name=\"type\"><option value=\"application\">Application service</option><option value=\"managed\">Managed backing service</option><option value=\"external\">External dependency</option></select></label><label>Name<input name=\"name\" required></label><label>Source<select name=\"source_type\"><option value=\"git\">Git</option><option value=\"image\">Image</option><option value=\"external\">External</option></select></label><label>Image<input name=\"image\"></label><label class=\"span2\">Repo URL<input name=\"repo_url\"></label><label>Branch<input name=\"branch\" placeholder=\"main\"></label><label>Git SHA<input name=\"git_sha\" required></label><label>Container port<input name=\"container_port\" type=\"number\" min=\"1\"></label><label>Health path<input name=\"health_path\" placeholder=\"/health\"></label><label>Replicas<input name=\"replicas\" type=\"number\" min=\"1\" value=\"1\"></label><p class=\"muted span2\">Deploy requires readiness, a healthy deploy-capable Agent, and a concrete Git SHA.</p><button class=\"span2 primary\">Save draft</button></form>";
}
...
function deploymentsTable(){
 ...
 return "<table class=\"table\"><thead><tr><th>Request</th><th>Service</th><th>Status</th><th>Plan</th><th>Target</th><th>Created</th><th>Actions</th></tr></thead><tbody>"+state.deployments.map(d=>"<tr><td>"+esc(d.id)+"</td><td>"+esc(serviceName(d.service_id))+"</td><td><span class=\"status "+statusClass(d.status)+"\">"+esc(d.status)+"</span></td><td>"+esc(shortHash(d.deployment_plan_hash))+"<br><span class=\"muted\">"+esc(d.rollback_eligible?"rollback ready":(d.rollback_blocked_reason||"no rollback"))+"</span></td><td>"+esc(d.node_id||"-")+"<br><span class=\"muted\">"+esc(d.agent_id||"-")+"</span></td><td>"+esc(rel(d.created_at))+"</td><td><button onclick=\"loadDeployEvents('"+esc(d.id)+"')\">Events</button> <button onclick=\"rollbackDeploy('"+esc(d.id)+"')\" "+(d.rollback_eligible?"":"disabled")+">Rollback</button></td></tr>").join("")+"</tbody></table>";
}
...
async function rollbackDeploy(id){try{const d=await api("/api/projects/"+state.project.id+"/deployments/"+id+"/rollback",{method:"POST",write:true,body:{requested_by:"ui"}});await load();await loadDeployEvents(d.id,false);switchView("deployments")}catch(e){toast(e.message)}}
